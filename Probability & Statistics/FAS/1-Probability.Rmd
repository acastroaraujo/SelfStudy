...
- More probability mass towards the left: $X \sim \text{beta}(1, 5)$

```{r, fig.height=1}
fX <- function(x) dbeta(x, 1, 5)
tibble(x = 0:1) %>% 
  ggplot(aes(x)) +
  geom_area(sta = "function", 
            fun = fX, 
            fill = "steelblue1") +
  labs(y = "density") +
...
- More probability mass towards the right: $X \sim \text{beta}(5, 1)$

```{r, fig.height=1}
fX <- function(x) dbeta(x, 5, 1)
tibble(x = 0:1) %>% 
  ggplot(aes(x)) +
  geom_area(sta = "function", 
            fun = fX, 
